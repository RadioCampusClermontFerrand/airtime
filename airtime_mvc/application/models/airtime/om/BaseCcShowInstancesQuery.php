 ...
 *
 * @method CcShowInstancesQuery leftJoinRotation($relationAlias = null) Adds a LEFT JOIN clause to the query using the Rotation relation
 * @method CcShowInstancesQuery rightJoinRotation($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Rotation relation
 * @method CcShowInstancesQuery innerJoinRotation($relationAlias = null) Adds a INNER JOIN clause to the query using the Rotation relation
 *
...
abstract class BaseCcShowInstancesQuery extends ModelCriteria
{
     ...
     * Example usage:
     * <code>
     * $query->filterByDbRotation(1234); // WHERE rotation = 1234
     * $query->filterByDbRotation(array(12, 34)); // WHERE rotation IN (12, 34)
     * $query->filterByDbRotation(array('min' => 12)); // WHERE rotation >= 12
     * $query->filterByDbRotation(array('max' => 12)); // WHERE rotation <= 12
     * </code>
     *
     * @see       filterByRotation()
     *
     * @param     mixed $dbRotation The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     ...
     * @return CcShowInstancesQuery The current query, for fluid interface
     */
    public function filterByDbRotation($dbRotation = null, $comparison = null)
    {
        if (is_array($dbRotation)) {
            $useMinMax = false;
            if (isset($dbRotation['min'])) {
                $this->addUsingAlias(CcShowInstancesPeer::ROTATION, $dbRotation['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($dbRotation['max'])) {
                $this->addUsingAlias(CcShowInstancesPeer::ROTATION, $dbRotation['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
     ...
     *
     * @param   Rotation|PropelObjectCollection $rotation The related object(s) to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CcShowInstancesQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
    ...
    public function filterByRotation($rotation, $comparison = null)
    {
        if ($rotation instanceof Rotation) {
            return $this
                ->addUsingAlias(CcShowInstancesPeer::ROTATION, $rotation->getDbId(), $comparison);
            ...
            return $this
                ->addUsingAlias(CcShowInstancesPeer::ROTATION, $rotation->toKeyValue('PrimaryKey', 'DbId'), $comparison);
        } else {
